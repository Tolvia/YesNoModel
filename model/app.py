...
CLASSES = ["si", "no", "ya", "que", "yo"]

logger.info("Iniciando la carga del modelo...")
try:
    model = tf.keras.models.load_model("./monosyllables_model_v0.keras")
...
def extract_mfcc(audio_data, sr=8000):
    ...
    logger.debug(f"MFCC calculado, forma: {mfcc.shape}")
    if mfcc.shape[1] < num_blocks:
        pad_width = num_blocks - mfcc.shape[1]
        mfcc = np.pad(mfcc, ((0, 0), (0, pad_width)))
        logger.debug(f"Rellenado a {num_blocks} bloques")
    elif mfcc.shape[1] > num_blocks:
